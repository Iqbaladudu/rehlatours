...
// Helper function to get form statistics (for admin dashboard)
export async function getFormStatistics() {
  try {
    const payload = await getPayload({ config })

    const totalSubmissions = await payload.count({
      collection: 'umrah-form',
    })

    const pendingReview = await payload.count({
      collection: 'umrah-form',
      where: {
    ...
    return {
      ...
      data: {
        ...
        pending: pendingReview.totalDocs,
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Terjadi kesalahan saat mengambil statistik',
    }
  }
}
